...
/**
 * Created by sam on 17-5-11.
 */

public class EsrPoiExporter {
    ...
    public EsrPoiExporter(List<?>... data) {
        this.data = data;
    }

    public static EsrPoiExporter data(List<?>... data) {
        return new EsrPoiExporter(data);
    }
    ...
    public Workbook export() {
        ...
        //initHeadCellStyle(wb.createCellStyle());//表头样式
    ...
    private static void processAsModel(String[] columns, Row row, Object obj) {
        ...
        Set<Map.Entry<String, Object>> entries = model._getAttrsEntrySet();
        if (columns.length == 0) { // show all if column not specified
            int columnIndex = 0;
    ...
    private static void processAsRecord(String[] columns, Row row, Object obj) {
        ...
        if (columns.length == 0) { // show all if column not specified
            ...
            for (int j = 0, len = columns.length; j < len; j++) {
                cell = row.createCell(j);
                Object cellValue = record.get(columns[j])== null ? "" :record.get(columns[j]);
                if (cellValue instanceof Number) {
                    cell.setCellValue(((Number) cellValue).doubleValue());
                } else {
                    cell.setCellValue(cellValue.toString());
    ...
    public EsrPoiExporter version(String version) {
        this.version = version;
        return this;
    }

    public EsrPoiExporter sheetName(String sheetName) {
        this.sheetNames = new String[]{sheetName};
        return this;
    }

    public EsrPoiExporter sheetNames(String... sheetName) {
        this.sheetNames = sheetName;
        return this;
    }

    public EsrPoiExporter cellWidth(int cellWidth) {
        this.cellWidth = cellWidth;
        return this;
    }

    public EsrPoiExporter headerRow(int headerRow) {
        this.headerRow = headerRow;
        return this;
    }

    public EsrPoiExporter header(String... header) {
        this.headers = new String[][]{header};
        return this;
    }

    public EsrPoiExporter headers(String[]... headers) {
        this.headers = headers;
        return this;
    }

    public EsrPoiExporter column(String... column) {
        this.columns = new String[][]{column};
        return this;
    }

    public EsrPoiExporter columns(String[]... columns) {
     ...
     */
    private void initHeadCellStyle(CellStyle headStyle) {
